...
import {MdOutlineErrorOutline, MdOutlineFileDownload, MdOutlineSortByAlpha} from "react-icons/md";
...
export default function FindLeads({data = true}) {
    ...
    const getFunction = async () => {
        setLoading(true)
        const leadData = await GetLeads(selectedType, selectedRadius, keyword, location);
    ...
    const sortLeadsA2Z = async () => {
        const leadData = await GetLeads(selectedType, selectedRadius, keyword, location, false);
        setLeads(leadData?.data)
        setSort(!sort)
    }
    const sortLeadsZ2A = async () => {
        const leadData = await GetLeads(selectedType, selectedRadius, keyword, location, true);
        setLeads(leadData?.data)
        setSort(!sort)
    }
    // Download //
    ...
    const handleDownlaod = async () => {
        ...
        try {
            const res = await fetch(
                ...
                {
                    method: "POST",
                    body: JSON.stringify({leads: selectedLeads.length === 0 ? leads : selectedLeads}),
                    headers: {
        ...
        <>
                ...
                <div className={'flex items-center'}>
                    <div className={"flex items-center"}>
                        <div className={"flex flex-row gap-4 py-1 px-5 items-center"}>
                            <Checkbox id="terms" className={'h-[17px] w-[17px] rounded-none border-bg-secondary-50'}
                        ...
                        <div className={'border-l border-gray-200 pl-3'}>
                            <Dropdown setSelectedType={setSelectedType} placeholder={'Type'}/>
                        </div>
                        {/*keywords*/}
                        <SearchBar
                            keywords={true}
                    ...
                    <div className={'pl-2'}>
                        <Dropdown setSelectedRadius={setSelectedRadius} placeholder={'Radius'}/>
                    </div>
                    <div className={'pl-2'}>
                        <Button className={'bg-primary-500 h-[36px]'}
                                disabled={!keyword || !location || !selectedType || !selectedRadius}
                                onClick={getFunction}>
                            {
                                loading ? (
            ...
            {  /*find leads table*/}
            {
                leads?.length !== 0 ? (
                    <>
                        <div
                            className="grid grid-cols-3 grid-rows-1 gap-4 p-3 bg-secondary-50 border rounded-b-[16px] cursor-pointer">
                            {
                                leads?.map((lead) => (
                                    <div
                                        className={'p-4 bg-white rounded-[10px] border bg-border-secondary-50'}
                                        key={lead.id}> {/*main div*/}
                                        <div className={'flex items-center gap-4 mb-4'}>
                                            <Checkbox id="terms" checked={selectedIds.includes(lead.id)}
                                                      className={'h-[17px] w-[17px] rounded-none border-bg-secondary-50'}
                                        ...
                                        <div className={'flex gap-6 items-center mb-2'}>
                                            <div className={'flex gap-2 items-center'}>
                                                ...
                                                <span
                                                    className={'text-16 font-normal'}>{lead?.phone ? lead?.phone : 'NA'}</span>
                                            </div>
                                            <div className={'flex gap-2 items-center'}>
                                                <MdMailOutline size={17} className={'text-secondary-400'}/>
                                                <span
                                                    className={'text-16 font-normal'}>{lead?.email ? lead?.email : 'NA'}</span>
                                            </div>
                                        </div>
                    ...
                    <div
                        className={'flex flex-col justify-center items-center p-10 border rounded-b-[16px] cursor-pointer gap-3'}>
                        <Image src={findLeads} alt={'findLeads'}/>
                        <span>Find active small business leads and places on google maps...</span>
                    </div>
                )
            }
        </>
